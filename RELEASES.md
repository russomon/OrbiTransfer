...
## v0.2.0 - 2026-09-18 — Renamed to OrbiTransfer + the chat / warm-transfer batch
- **Renamed OrbitXfer → OrbiTransfer everywhere.** App + CLI, bundle id `com.orbitolive.orbitransfer.tauri`, wire-protocol ALPNs (`orbitransfer/chat/1`, `orbitransfer/label/0`, `orbitransfer/pair/1`), env vars (`ORBITRANSFER_*`), store dirs (`.orbitransfer-*`), and the GitHub repo (`russomon/OrbiTransfer`). **Clean break:** OrbiTransfer does not interoperate with OrbitXfer v0.1.x builds.
- **Independent, bidirectional chat that outlives the transfer** (folds in v0.1.85–0.1.92). Chat rides its own ALPN, either side can reconnect, and Stop Send / Stop Receive no longer drops the conversation. Plus receivers-list dedup and snappier reconnect.
- **Chat heartbeat + warm resume** (v0.1.93–0.1.94). 3 s ping / 8 s idle detection; Stop → Resume re-runs the transfer in the SAME process (no respawn), so chat is undisturbed — on both sender and receiver.
- **Status accuracy fixes** (v0.1.95–0.1.97). Resumed transfers show the receiver row correctly (handle non-monotonic connection ids), a second folder can be shared without dropping chat, and a successful (including resumed/partial) transfer resolves to ✓ Transfer Complete instead of a spurious "disconnected".
- **Warm in-process new-receive** (v0.1.98). Accept a second ticket without a cold respawn — chat stays connected and the already-warm endpoint is reused.
- Signed + notarized **universal (Intel + Apple Silicon)** macOS build; the release build script now auto-notarizes and staples the `.dmg`.
...
- **How the protocol works.** New ALPN `orbitransfer/pair/1`. The sender's ephemeral NodeID is BLAKE3-derived (domain-separated) from the code, so the receiver knows where to dial without any directory service — iroh's existing relay-based discovery introduces the two peers. The handshake is **CPace** (a modern PAKE; crate `pake-cpace`) over a session key neither side knows in advance, with **Argon2id** (64 MiB × 3 iters) wrapping the code before CPace consumes it. That makes every PAKE attempt — honest or malicious — pay a ~1 second memory-hard cost. Combined with a per-source 5-attempts-per-60-s rate limit on the host, 51.7 bits of code entropy translates to roughly **74,000 years** to brute-force solo. Off-dictionary codes fail at validation; wrong-but-format-valid codes derive a different NodeID and fail at iroh discovery with **no information leakage**.
...
- **New CLI subcommands** (both files and folders work end-to-end):
  - `orbitransfer-iroh-cli pair-host <file-or-folder>` — hashes via the same `prepare_single_file` / `prepare_folder` helpers `send` uses, generates a code, registers both `PAIR_ALPN` and `iroh_blobs::ALPN` on the same Router, holds `TempTag`s for the session, waits for Ctrl+C. Multiple receivers per session, like classic Send.
  - `orbitransfer-iroh-cli pair-join <code> <output-path>` — validates and normalizes the code, derives the sender's NodeID, dials on `PAIR_ALPN`, runs the CPace handshake, decrypts the offer, then **hands off to the existing `run_receive`** to perform the actual download. Zero refactor risk to the v0.1.83 receive pipeline (preflight, retry, progress events, folder extraction, integrity checks, auto store cleanup all reused unchanged).
...
## v0.1.81 - 2026-05-31
- **Aggressive size-optimized Rust release profile.** No app code changes — only `[profile.release]` settings in both `OrbiTransfer-iroh-cli/Cargo.toml` and `OrbiTransfer-iroh-tauri/src-tauri/Cargo.toml`:
...
## v0.1.80 - 2026-05-29
- **Dark mode — proper, system-aware, with a manual override.** OrbiTransfer now ships a coherent light/dark palette and a three-state theme picker under **View → Theme → {Auto (Follow System), Light, Dark}**.
...
## v0.1.76 - 2026-05-29
- **One universal Mac DMG instead of two arch-specific ones.** CI now produces a single `OrbiTransfer_<version>_universal.dmg` that runs natively on both Apple Silicon and Intel Macs — same signed/notarized DMG works everywhere. Eliminates the dependency on GitHub's `macos-13` Intel runner pool, which had become unreliable (multi-hour queues, sometimes days). No app code changes from v0.1.75.
...
## v0.1.75 - 2026-05-28
- **CI now builds a signed Intel Mac DMG** alongside the existing Apple Silicon one. GitHub's `macos-latest` is now Apple Silicon (macos-14), so every release through v0.1.74 shipped arm64-Mac-only. The build matrix now includes `macos-13` (Intel) too, with the same signing + notarization path, producing a second signed/notarized DMG per tag release (`OrbiTransfer_<version>_x64.dmg`).
- Artifact names are arch-aware (`OrbiTransfer-macOS-ARM64` vs `OrbiTransfer-macOS-X64`) so the two Mac matrix entries don't collide in the release job.
...
- **See who's downloading (opt-in receiver labels + per-receiver rows).** The sender now shows a "Receivers" panel listing everyone currently pulling the file, each with its own progress bar, %, bytes, and speed. Multiple simultaneous receivers are tracked independently (replacing the single jumpy upload bar during the upload phase).
  ...
  - **How it works:** a new side-channel ALPN (`orbitransfer/label/0`) registered on the sender's router. The receiver opens a short connection and sends the label; the sender correlates it to the receiver's cryptographically-authenticated NodeID, so a peer can only label *itself*, not impersonate another. Best-effort and backward-compatible — an older sender without the protocol just declines, and the download proceeds normally.
...
- **CLI gets `expected_size` plumbed end-to-end.** Tauri's `start_receive` command now takes an optional `expectedSize` parameter and forwards it as `ORBITRANSFER_EXPECTED_SIZE` to the sidecar. The CLI already honored that env var (since pre-v0.1.64); we just hadn't been setting it. The CLI emits `download_size` immediately from the seed and only re-emits it if the provider's `observe()` returns a different number.
...
- **`start_receive` accepts an explicit `store_dir` override.** Optional `storeDir` parameter (no UI yet — forward-compat for resume / advanced flows). When set, it forwards as `ORBITRANSFER_STORE_DIR` to the sidecar; the CLI honors the path verbatim and skips the default auto-cleanup. The default behavior (fresh receive, no override) is unchanged: `<destination>.orbitransfer-pieces/` next to the chosen destination, auto-cleaned on success.
...
- **Receive cache returns to the v0.1.55 design.** The Tauri sidecar no longer sets `ORBITRANSFER_STORE_DIR` for Receive invocations — only for Send, where per-window FsStore isolation actually matters. As a result, receives now cache in `<destination>.orbitransfer-pieces/` next to the file you're saving (visible, not hidden in `<app-data>`) AND the CLI's existing auto-cleanup deletes that folder after the export completes. No more multi-GB cached copies lingering in `<app-data>` after every receive.
- **Received file always keeps the original filename.** If the share ticket includes a filename and you picked a destination with a different name (e.g. "Untitled" because the save dialog defaulted before you pasted the ticket), the CLI now writes the file with the ticket's filename in the folder you chose. You pick where; OrbiTransfer fills in what.
...
## v0.1.62 - 2026-05-19
- **Full menu bar.** Adds File, View, and Window menus to the OrbiTransfer / Edit pair we already had.
  ...
  - **Window**: Minimize (⌘M), Zoom, plus a dynamic list of every currently-open OrbiTransfer window. Each entry is `<title> (<n>)` so windows sharing a title can still be told apart. Selecting an entry brings that window to the front. The list rebuilds automatically when windows open or close.
...
- **Connection mode** is back. The Send panel now has two radio buttons — "Direct + relay fallback (recommended)" and "Direct only (no relay)" — matching the Electron app's behavior. Selection persists across launches (and across windows) via localStorage. Plumbed through to the sidecar via `ORBITRANSFER_TICKET_MODE`. The explanatory copy under the radios matches the Electron version: *"Transfers are end-to-end encrypted in both modes. Direct-only disables relay fallback, and is true peer-to-peer with no relay server used."*
...
- New CLI behavior: `start_send` Tauri command now accepts an optional `connection_mode` parameter and forwards it via the `ORBITRANSFER_TICKET_MODE` env var on the sidecar. Receive sidecars continue to ignore this env var.
...
- CLI: new `ORBITRANSFER_PER_FILE_IDENTITY_DIR` env var. When set, the send flow uses the file's BLAKE3 hash to derive `<dir>/<hash>.key` as the identity location, falling back to legacy `ORBITRANSFER_KEY_PATH` / ephemeral if not set. Standalone terminal CLI usage without this env var is unchanged (still fully ephemeral by default).

## v0.1.60 - 2026-05-19
- Persistent OrbiTransfer identity. The iroh identity key is now stored at `<app-data>/identity.key` and reused across every send. Same file → same share ticket. Old tickets stay live as long as you're actively serving that file in the current session; the FsStore is still wiped between sessions, so old tickets can only fetch blobs you re-pick this session.
- New **OrbiTransfer → Reset Identity…** menu item. Confirms via dialog, then: kills every in-flight transfer in every window, deletes `identity.key`, and wipes every per-window store. The next launch generates a fresh Node ID. After a reset:
...
- Internal: Tauri sidecar spawns set `ORBITRANSFER_KEY_PATH=<app-data>/identity.key` so the CLI loads the persistent key instead of generating an ephemeral one each run.
...
- Behavior preserved across the migration: lenient share-ticket parser (accepts the full `orbitransfer-iroh-cli receive blob… /path` line, not just the bare token), share-line filename preservation with auto-fill destination in the Receive panel, resumable last-send and last-receive (persisted to localStorage, surfaced as Resume pills), quit warnings on Cmd-W and Cmd-Q while a transfer is active, per-window Send/Receive mode switch.
- macOS bundles are signed with the project's Developer ID and notarized by Apple — Gatekeeper accepts the DMG without any bypass on the receiving Mac. Build/sign/notarize flow documented in `OrbiTransfer-iroh-tauri/README.md`; per-build env config in `tauri.env.example`.
...
- Bonus CLI fix bundled in (originally v0.1.57): the standalone `orbitransfer-iroh-cli` watches stdin for EOF and exits when its parent dies. Eliminates the "ticket_hashing_start hangs forever" class of bugs caused by orphan sidecars holding `~/.orbitransfer-store`'s exclusive flock.
...
- Fixes the recurring "ticket_hashing_start hang" caused by previous orphans still holding `~/.orbitransfer-store/blobs.db` (or any explicit `ORBITRANSFER_STORE_DIR`).
...
## v0.1.55 - 2026-04-17
- Renamed the managed receive store from the hidden `.orbitransfer-store` folder to a visible per-destination folder named `<filename>.orbitransfer-pieces`.
...
## v0.1.54 - 2026-04-16
- Made receive parsing more forgiving so OrbiTransfer can recover wrapped or embedded share tokens from surrounding text instead of rejecting them outright.
...
- Fixed resumed transfer counters so OrbiTransfer restores saved progress in the UI and the receiver now reports already-downloaded local bytes before download resumes.
...
## v0.1.51 - 2026-04-14
- Added multi-window sessions so separate OrbiTransfer windows can send and receive files in parallel.
