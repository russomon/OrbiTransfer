...
- Immediate next action: On the next feature or release task, pull `origin/main`, read `AGENTS.md`, `CLAUDE.md`, `CURRENT_WORK.md`, `NEXT_STEPS.md`, and `DECISIONS.md`, then proceed in OrbiTransfer rather than `orelay`.
...
- OrbiTransfer is the active evolution path for the older `orelay` work.
- GitHub remote is configured at `https://github.com/russomon/OrbiTransfer.git`.
...
- Who should pick this up next: Current OrbiTransfer maintainer
